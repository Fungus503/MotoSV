  ...
  return (
    <header className="h-16 flex items-center justify-between px-6 sticky top-0 z-30" style={{ backgroundColor: 'var(--bg-card)', borderBottom: '1px solid var(--border-color)' }}>
      <div className="flex items-center gap-4">
        <button onClick={onToggleSidebar} className="lg:hidden p-2 hover:bg-gray-100 rounded-lg" style={{ color: 'var(--text-primary)' }}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 12h18M3 6h18M3 18h18"/></svg>
        </button>
        <div className="relative hidden sm:block">
          <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: 'var(--text-secondary)' }} />
          <input placeholder={t('header.search')} name="search" autoComplete="off" className="pl-9 pr-4 py-2 rounded-lg text-sm w-64 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" style={{ backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border-color)', color: 'var(--text-primary)' }} />
        </div>
      </div>
      <div className="flex items-center gap-2">
        <button onClick={() => setDarkMode(!darkMode)} className="p-2 hover:bg-gray-100 rounded-lg" style={{ color: 'var(--text-secondary)' }} title={t(darkMode ? 'header.lightMode' : 'header.darkMode')}>
          {darkMode ? <Sun size={18} /> : <Moon size={18} />}
        </button>
        <button onClick={() => i18n.changeLanguage(i18n.language === 'es' ? 'en' : 'es')} className="p-2 hover:bg-gray-100 rounded-lg text-sm font-medium" style={{ color: 'var(--text-secondary)' }} title={i18n.language === 'es' ? t('header.english') : t('header.spanish')}>
          {i18n.language === 'es' ? t('header.langEn') : t('header.langEs')}
        </button>

        <div className="relative" ref={notifRef}>
          <button onClick={() => setShowNotifications(!showNotifications)} className="p-2 hover:bg-gray-100 rounded-lg relative" style={{ color: 'var(--text-secondary)' }} title={t('header.notifications')}>
            <Bell size={18} />
            {(alerts?.length ?? 0) > 0 && (
        ...
        <button onClick={() => navigate('/app/chats')} className="p-2 hover:bg-gray-100 rounded-lg" style={{ color: 'var(--text-secondary)' }} title={t('header.chat')}>
          <MessageSquare size={18} />
        </button>

        <div className="relative" ref={profileRef}>
          <button onClick={() => setShowProfile(!showProfile)} className="flex items-center gap-2 ml-2 p-1.5 hover:bg-gray-100 rounded-lg">
          ...
          {showProfile && (
            <div className="absolute right-0 top-full mt-1 w-48 rounded-xl border shadow-lg py-2 z-50" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
              <button onClick={() => { navigate('/app/profile'); setShowProfile(false) }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-gray-50" style={{ color: 'var(--text-primary)' }}>
                <User size={16} /> {t('header.editProfile')}
              </button>
              <button onClick={onSignOut} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-red-600 hover:bg-gray-50">
