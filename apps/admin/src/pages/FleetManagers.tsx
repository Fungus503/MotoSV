...
export function FleetManagersPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['admin-fleet-managers'],
    queryFn: async () => {
      const { data, error } = await supabase.from('fleet_managers').select('*, user:user_id(full_name, phone)').order('created_at', { ascending: false })
  ...
  return (
    <div>
      ...
      <DataTable columns={[
        { key: 'user', label: t('fleetManagers.name'), sortable: true, render: (f: any) => f.user?.full_name ?? '—' },
        { key: 'company_name', label: t('fleetManagers.company') },
        { key: 'is_verified', label: t('fleetManagers.verified'), render: (f: any) => <StatusBadge status={f.is_verified ? 'approved' : 'pending'} /> },
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSave} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2"><label htmlFor="fm-name" className="block text-sm font-medium text-gray-700 mb-1">{t('fleetManagers.name')}</label><input id="fm-name" value={form.company_name} onChange={(e) => setForm({ ...form, company_name: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" required /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label htmlFor="fm-email" className="block text-sm font-medium text-gray-700 mb-1">{t('fleetManagers.email')}</label><input id="fm-email" type="email" value={form.company_email} onChange={(e) => setForm({ ...form, company_email: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" /></div>
                <div><label htmlFor="fm-phone" className="block text-sm font-medium text-gray-700 mb-1">{t('fleetManagers.phone')}</label><input id="fm-phone" value={form.company_phone} onChange={(e) => setForm({ ...form, company_phone: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" /></div>
              </div>
              <div><label htmlFor="fm-tax" className="block text-sm font-medium text-gray-700 mb-1">{t('fleetManagers.taxId')}</label><input id="fm-tax" value={form.tax_id} onChange={(e) => setForm({ ...form, tax_id: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" /></div>
