  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => { setForm({ category_id: cats?.[0]?.id ?? '', name: '', slug: '', base_fare: 2, per_km: 0.5, per_min: 0.1, min_fare: 3 }); setShowForm(true) }}
      ...
      <DataTable
        columns={[
          ...
          { key: 'actions', label: '', width: 'w-16', render: (t) => (
            <button onClick={async (e) => { e.stopPropagation(); try { if (confirm(t('common.confirmDelete'))) await deleteType.mutateAsync(t.id) } catch (e) { alert(handleError(e)) } }}
        ...
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSave} className="space-y-3">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('serviceTypes.category')}</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t('serviceTypes.slug')}</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t('serviceTypes.baseFareLabel')}</label>
                  <input type="number" step="0.01" value={form.base_fare} onChange={(e) => setForm({ ...form, base_fare: +e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t('serviceTypes.perKmLabel')}</label>
                  <input type="number" step="0.01" value={form.per_km} onChange={(e) => setForm({ ...form, per_km: +e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t('serviceTypes.perMinLabel')}</label>
                  <input type="number" step="0.01" value={form.per_min} onChange={(e) => setForm({ ...form, per_min: +e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t('serviceTypes.minFareLabel')}</label>
                  <input type="number" step="0.01" value={form.min_fare} onChange={(e) => setForm({ ...form, min_fare: +e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
                </div>
              </div>
