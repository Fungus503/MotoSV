  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button type="button" onClick={() => setShowBlock(true)} className="flex items-center gap-2 bg-red-600 text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-red-700">
      ...
      <DataTable
        columns={[
          ...
          { key: '', label: '', width: 'w-16', render: (r: any) => (
            <button type="button" onClick={async (e) => { e.stopPropagation(); try { if (confirm(t('blockedUsers.unblockBtn') + '?')) await unblockMutation.mutateAsync(r.id) } catch (e) { alert(handleError(e)) } }}
      ...
      />
      {showBlock && (
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center" onClick={() => setShowBlock(false)} role="presentation" onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setShowBlock(false) }}>
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={async (e) => {
              ...
              <div>
                <label htmlFor="blocked-user" className="block text-sm font-medium text-gray-700 mb-1">{t('blockedUsers.name')}</label>
                <select id="blocked-user" value={blockForm.user_id} onChange={(e) => setBlockForm({ ...blockForm, user_id: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" required>
                  <option value="">—</option>
                  {(allUsers ?? []).map((u: any) => <option key={u.id} value={u.id}>{u.full_name ?? u.email} ({u.role})</option>)}
                </select>
              </div>
              <div>
                <label htmlFor="blocked-reason" className="block text-sm font-medium text-gray-700 mb-1">{t('blockedUsers.reasonLabel')}</label>
                <textarea id="blocked-reason" value={blockForm.reason} onChange={(e) => setBlockForm({ ...blockForm, reason: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" rows={3} required />
              </div>
