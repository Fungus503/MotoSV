...
export function FleetVehiclesPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('fleet_vehicles').select('*, fleet_manager:fleet_manager_id(company_name), type:vehicle_type_id(name)').order('created_at', { ascending: false })
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['admin-fleet-vehicles'] }),
  })
  ...
  return (
    <div>
      ...
      <DataTable columns={[
        ...
        { key: 'type', label: t('fleetVehicles.type'), render: (v: any) => v.type?.name ?? '—' },
        { key: 'capacity', label: t('fleetVehicles.capacity'), render: (v: any) => `${v.capacity}${t('fleetVehicles.pax')}` },
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSave} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div><label htmlFor="fv-make" className="block text-sm font-medium text-gray-700 mb-1">{t('fleetVehicles.make')}</label><input id="fv-make" value={form.make} onChange={(e) => setForm({ ...form, make: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" required /></div>
                <div><label htmlFor="fv-model" className="block text-sm font-medium text-gray-700 mb-1">Modelo</label><input id="fv-model" value={form.model} onChange={(e) => setForm({ ...form, model: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label htmlFor="fv-plate" className="block text-sm font-medium text-gray-700 mb-1">{t('fleetVehicles.plate')}</label><input id="fv-plate" value={form.plate_number} onChange={(e) => setForm({ ...form, plate_number: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" /></div>
                <div><label htmlFor="fv-capacity" className="block text-sm font-medium text-gray-700 mb-1">{t('fleetVehicles.capacity')}</label><input id="fv-capacity" type="number" min={1} value={form.capacity} onChange={(e) => setForm({ ...form, capacity: +e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label htmlFor="fv-year" className="block text-sm font-medium text-gray-700 mb-1">Año</label><input id="fv-year" type="number" min={2000} max={2030} value={form.year} onChange={(e) => setForm({ ...form, year: +e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" /></div>
                <div><label htmlFor="fv-color" className="block text-sm font-medium text-gray-700 mb-1">Color</label><input id="fv-color" value={form.color} onChange={(e) => setForm({ ...form, color: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" /></div>
              </div>
