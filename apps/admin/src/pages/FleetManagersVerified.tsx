...
export function FleetManagersVerifiedPage() {
  const { t, i18n } = useTranslation()
  const qc = useQueryClient()
  
  const { data, isLoading } = useQuery({
    queryKey: ['admin-fleet-managers-verified'],
    queryFn: async () => {
      const { data, error } = await supabase.from('fleet_managers').select('*, user:user_id(full_name, phone)').eq('is_verified', true).order('created_at', { ascending: false })
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['admin-fleet-managers-verified'] }),
  })
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSave} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2"><label htmlFor="fmv-name" className="block text-sm font-medium text-gray-700 mb-1">{t('fleetManagers.name')}</label><input id="fmv-name" value={form.company_name} onChange={(e) => setForm({ ...form, company_name: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" required /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label htmlFor="fmv-email" className="block text-sm font-medium text-gray-700 mb-1">{t('fleetManagers.email')}</label><input id="fmv-email" type="email" value={form.company_email} onChange={(e) => setForm({ ...form, company_email: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" /></div>
                <div><label htmlFor="fmv-phone" className="block text-sm font-medium text-gray-700 mb-1">{t('fleetManagers.phone')}</label><input id="fmv-phone" value={form.company_phone} onChange={(e) => setForm({ ...form, company_phone: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" /></div>
              </div>
              <div><label htmlFor="fmv-tax" className="block text-sm font-medium text-gray-700 mb-1">{t('fleetManagers.taxId')}</label><input id="fmv-tax" value={form.tax_id} onChange={(e) => setForm({ ...form, tax_id: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" /></div>
